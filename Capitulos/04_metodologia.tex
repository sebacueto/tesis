...
Simulación Inventor & $98890.45$ [N/m] & $1809049.41$ [N/m] \\
Cálculo & $9.216\cdot 10^5$ [N/m] & -- \\ \hline
\end{tabular}
...
\end{tabular}
\caption{Datos utilizados del ensayo de esfuerzo-deformación del acero AISI 1020 HR en el modelo MISO. Fuente: }
\label{tab:esfdef_util}
\end{table}

Por otro lado, para la deformación elástica se usará un modelo de elasticidad $E =$ 200 GPa y el esfuerzo de fluencia $\sigma_y=$ 293,5 MPa.
...
\subsubsection{Malla}
Producto de lo irregular de la geometría de la probeta y la importancia que tienen para los resultados la zona media, por sobre los agarres, es que se opta por mallar para cada sección. En los parámetros generales del mallado se utilizaron parámetros que buscaran mejorar su calidad. \textcolor{red}{referencia al manual de ansys}
...
Para obtener los resultados de una malla que se adaptara a los requisitos, primero se utiliza el \textit{body sizing} y el \textit{face sizing}. Así, la configuración utilizada para cada sección es la siguiente:
\begin{itemize*}
...
\begin{itemize*}
	...
	\item Esfuerzo máximo normal en dirección \textit{x} ($\sigma_{xx}$), según referencia \ref{fig:cond_borde}.
	...
	\item Deformación unitaria normal elástica ($\varepsilon_{xx}$)
\end{itemize*}
