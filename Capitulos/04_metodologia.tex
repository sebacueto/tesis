...
Con esto, la probeta a ensayar estará sometida a un esfuerzo en flexión, empotrada por la mordaza del brazo de carga y la mordaza empotrada a la estructura de la máquina, ambas mostradas en la figura \ref{fig:mordazas}.
...
\subparagraph{Probeta.} La probeta utilizada actualmente para el ensayo de fatiga es de acero. Su geometría, como se aprecia en la figura \ref{fig:probeta}, consiste en una pequeña viga de largo 3\nicefrac{1}{2}'', de sección cuadrada en sus extremos y una entalladura en el medio de sección circular, de lado \nicefrac{1}{2}'' de lado y 0,3'' de diámetro respectivamente.
...
\caption{Identificación de las distintas partes de madera de la estructura.}
\label{fig:mesa_iso}
\end{figure}
...
\end{gather}
\end{subequations}
Por otro lado, la tensión de trabajo en cizalle se obtiene a partir de la ecuación \ref{eq:f_cz} y la de diseño en cizalle por \ref{eq:cz_dis}. Dado que $K_r=1$ al no haber rebaje de la viga, entonces el valor obtenido para ambas tensiones son:
\begin{align}
...
\section{Modelo del sistema vibratorio}
...
\end{tabular}
\caption{Valores obtenidos de la rigidez de las vigas en voladizo y la probeta de acero}
\label{tab:k_valor}
\end{table}
...
\end{equation}
Para las vigas en voladizo, el segundo momento de área se calculó en la sección \ref{sec:mediciones_met}.
...
\begin{figure}[h]
\centering
\includegraphics[width=0.8\linewidth, trim={8cm 5cm 12cm 3cm},clip]{Imagenes/anglep_function.pdf}
...
Para resolver el sistema de ecuaciones que se obtuvo en \ref{eq:mov_matriz}, se utilizará el solver de ecuaciones diferenciales ordinarias de MATLAB, ode45, que se basa en el método de Runge-Kutta con un espacio de tiempo variable para resolver sistemas de EDOs de primer orden y con condición inicial. \cite{ode45}
...
\begin{itemize*}
	\item Probeta completa
		...
		\end{itemize*}
	\item Sección intermedia (figura \ref{fig:zona_media})
		\begin{itemize*}
			\item Face sizing
			\item Tamaño elemento: 0,001 mm
		\end{itemize*}
	\item Sección transversal media (figura \ref{fig:cara_corte})
		\begin{itemize*}
			\item Face sizing
...
\begin{itemize*}
	...
	\item Deformación unitaria normal elástica ($\varepsilon_{x}$),  según referencia \ref{fig:cond_borde}.
\end{itemize*}
