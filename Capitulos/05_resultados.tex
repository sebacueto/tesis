...
\caption{Distribución de la carga máxima $F_{max}$ versus la diferencia de masas $\Delta m$ de cada combinación de contrapesos, con $\omega_{max} =$ 25 Hz. Resultados en el anexo \ref{sec:anexob2}}
\label{fig:fmax_dm25}
\end{figure}
...
De manera análoga, se puede estudiar la influencia de la velocidad de giro máxima del disco desbalanceado sobre el movimiento del sistema. Las figuras en \ref{fig:f_w520}, muestran $F(t)$ para dos velocidad distintas, $\omega_{max,a} = 5$ [rad/s] y $\omega_{max,b}=20$ [rad/s]. 
...
\begin{gather*}
	N_f = \frac{1}{2} \left[\frac{\sqrt{\sigma_{vm}\cdot \sigma_a}}{\sigma'_f} \right]^{1/b} = \frac{1}{2} \left[\frac{\sqrt{13.81\cdot 5.06}}{896} \right]^{-1/0.12} \\
	N_f = 4.1378 \cdot 10^{16} ciclos
\end{gather*}
